...
const
  NumH150stars=149;            {Numbner of stars of Hipparcos 15 catalog - actually 149 }
...
type
  ...
  RStarH150=record
    ...
    RA,Decl:Double;        // coordinates ( Right Ascension and Declination, J2000 )
    PM_RA,PM_Dec:single;   // proper motion (in arcsecs/year J2000)
    Mag:Single;            // Magnitude ( small number --> brighter star )
    HipNum:integer;        // Hipparcos number
    HD:integer;            // what is that ?
    Parallax:double;
    TPM:double;            // total proper motion
    TrVel:double;          // transverse velocity
  end;

  TStarH150  = Class(TObject)
  private
    // current coordinates
    ...
    fDpmra,fDpmDec,fDpm:double;  //proper motion accumulated effects (arcsecs)

    RAi,Decli:Double;             //Coordinates in initial Epoch  ( J2000.0 )
    DAlfai,DDeltai:Double;       //calculate corrections from initial Epoch ( J2000.0 ) }
  ...
  public
    ...
    fRA,fDecl:Double;      // coordinates J2000
    ...
    Procedure   GetObjectData(SL:TStrings);  // return sumary of star data to Memos
    Property    GMT:TDateTime    read fGMT write SetGMT;  // set UT time ti calc coordinates ( UT and GMT are the same thing )
  end;

// catalog Hipparcos 150 objects
var StarsH150:Array[1..NumH150stars] of TStarH150;  //static db
...
begin
  ...
  fpm_RA :=StarRec.PM_RA/1000/15/Cosg(Decli);  //15 converte de graus p/ horas (ou vice-versa)
...
Procedure TStarH150.SetGMT(Value:TDateTime);
...
Procedure TStarH150.CalcDeclination_GHA(D,M,A,H:Double; var aDecl,aGHA:Double); // calc gha,decl
...
Function  TStarH150.AlreadyCalculatedToday(aDJ:Double):Boolean;
begin
  AlreadyCalculatedToday:= ( Round(aDJ)=Round(fLastCoordCalculation) );
end;


Procedure TStarH150.CalcCoordinates(aDia,aMes,aAno,aHora:Double);
...
begin
  if not fAlreadyCalculatedInitialCoordinates then {Ve se calcula coordenada iniciais da estrela}
    begin
      CalculaJ2000Correction(1.0,1.0,InitEpochH150,0.0,DAlfai,DDeltai); {Calcula corr da data inic. em rel a J2000.0 }
      fAlreadyCalculatedInitialCoordinates:=TRUE;
    end;

  DJ := JD(aAno,aMes,aDia,aHora);               // Julian date
  if AlreadyCalculatedToday(DJ) then exit;     // already calculado today, no need to recalculate slow changfing coordinates for the same day
  fLastCoordCalculation := DJ;

  CalculaJ2000Correction(aDia,aMes,aAno,aHora,DAlfa,DDelta); {Calcula correcoes}
  { (-) correcoes ref. ao inst. em que as coordenadas foram extraidas do
   Anuario Astronomico  (DDAlfai e DDeltai) =1993.0 }

  DAlfa  := DAlfa-DAlfai;     {Subtrai correcoes relativas ao instante inicial}
  DDelta := DDelta-DDeltai;

  //jul/04- calcula correcao relativa ao proper motion
  pm_anos := (aAno+aMes/12-InitEpochH150);    //diff in relation to J2000 (time of coord table) in years

  // proper motion factors
  fDpmra  := fpm_RA *pm_anos/3600*15;    //conv de arcseconds para graus ( 15 ??? )
  fDpmDec := fpm_Dec*pm_anos/3600;

  fRA  := RAi+  DAlfa /3600 + fDpmra;  {Aplica correcoes - Deltas em " --> /3600}
  fDecl:= Decli+DDelta/3600 + fDpmDec;
...
begin
  T := TJ2000(aAno, aMes, aDia, aHora);  {Tempo em seculos desde J2000}

  CorrectionForAberration(T,RAi,Decli,DAlfaAbe,DDeltaAbe);    {Calcula correcoes}
  CorrectionForNutation  (T,RAi,Decli,DAlfaNu,DDeltaNu);
  CorrectionForPrecession(T,RAi,Decli,DAlfaPre,DDeltaPre);
  {Sum correction}
  aDAlfa  := DAlfaNu  + DAlfaPre  + DAlfaAbe;     {Soma correcoes calculadas}
  aDDelta := DDeltaNu + DDeltaPre + DDeltaAbe;
...
Procedure TStarH150.CalculaSunTrueLongitude(T:Double;var Teta:Double);
...
Procedure TStarH150.CorrectionForAberration(T,aRAi,aDecli:Double;var DAlfaAbe,DDeltaAbe:Double);
...
Procedure TStarH150.CorrectionForPrecession(T,aRAi,aDecli:Double;var DAlfaPre,DDeltaPre:Double);
